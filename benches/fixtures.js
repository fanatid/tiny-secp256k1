...
export const fpoints = {
  ...
  pointAdd: _fpoints.valid.pointAdd.map((f) => ({
    P: Buffer.from(f.P, "hex"),
    Q: Buffer.from(f.Q, "hex"),
  })),
  pointAddScalar: _fpoints.valid.pointAddScalar.map((f) => ({
    P: Buffer.from(f.P, "hex"),
    d: Buffer.from(f.d, "hex"),
  })),
  ...
  pointMultiply: _fpoints.valid.pointMultiply.map((f) => ({
    P: Buffer.from(f.P, "hex"),
    d: Buffer.from(f.d, "hex"),
  })),
};
...
export const fprivates = {
  ...
  privateAdd: _fprivates.valid.privateAdd.map((f) => ({
    d: Buffer.from(f.d, "hex"),
    tweak: Buffer.from(f.tweak, "hex"),
  })),
  privateSub: _fprivates.valid.privateSub.map((f) => ({
    d: Buffer.from(f.d, "hex"),
    tweak: Buffer.from(f.tweak, "hex"),
  })),
};
